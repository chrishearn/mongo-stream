  ...
  async dumpCollection() {
    ...
    while (await cursor.hasNext()) {
      ...
      if (bulkOp.length !== 0 && bulkOp.length % (CollectionManager.elasticManager.bulkSize * 2) === 0) {
        ...
        logger.info(`${this.collectionName} progress: ${this.dumpProgress.count}/${totalCount} (${perc}%) -- tick: ${tickRate} docs/sec -- avg: ${rate} docs/sec -- eta ${eta}s`);
    ...
    this.dumpProgress.count += (bulkOp.length/2);
    logger.info(`${this.collectionName} FINAL request progress: ${this.dumpProgress.count}/${totalCount}`);
    await currentBulkRequest;
