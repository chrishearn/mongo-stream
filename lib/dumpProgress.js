...
class DumpProgress {
  ...
  async getFromCollection() {
    try {
      ...
      logger.error(`dumpProgress for ${this.collection} could not be retrieved from database`);
      logger.debug(err);
      this.reset();
    }
  }
  ...
  async writeToCollection() {
    try {
      ...
      logger.error(`resumeToken for collection ${this.collection} could not be saved to database`);
      logger.debug(err);
    }
  }
